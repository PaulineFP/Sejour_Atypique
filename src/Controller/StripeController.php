...
class StripeController extends AbstractController
{
   ...
   */
  public function startPayment(SessionInterface $session, HebergementRepository $repo, PanierRepository $panierRepo, ReservationsRepository $reservationsRepo, EntityManagerInterface $entitymanager)
  {
    ...
    //J'instancie le contenue de mon panier pour ensuite récupérer les informations
    foreach ($reservations as $ReservationId) {
      ...
      //Identification du prix unitaire:               
      $promo_checked = $reservation->getHebergement()->getIsPromotional();

      if ($promo_checked == 1) {
        $promo = $reservation->getHebergement()->getPromoTotal();
        $prix = $promo;
      } else {
        $prix = $reservation->getHebergement()->getTarif();
      }

      //j'épluche le contenue du panier à réutiliser
      $panierItems[] = [
        "quantity" => $nuits,
      ...
      $productDetail = $nuits . ' ' . $hebergement;
      ...
      'line_items'                  => [
        ...
        array_map(fn ($panierItems) => [
          ...
          'price_data' => [
            'currency'     => 'EUR',
            'product_data' => [
              'name' => $panierItems['hebergement']
            ],
            //stripe considère tout sous forme de centime
            'unit_amount'  => $panierItems['prix'] * 100
          ]
        ], $panierItems)
      ],
